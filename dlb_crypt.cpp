...
dlb_crypt::dlb_crypt(String key_) {
  for (int i = 0; i < key_.length(); i++) {
      this->key[i]=key_[i];
  }
}

void dlb_crypt::bufferSize(char* text, int &length)
{
  int i = strlen(text);
...
void dlb_crypt::encrypt(char* plain_text, char* output, int length)
{
  byte enciphered[length];
  AES aesEncryptor(this->key, this->iv, AES::AES_MODE_128, AES::CIPHER_ENCRYPT);
  aesEncryptor.process((uint8_t*)plain_text, enciphered, length);
  Serial.print("AES -> ");
  for (int i = 0; i < length; i++) {
      Serial.print(enciphered[i],HEX);
      Serial.print(" ");
  }
  Serial.println("");
...
void dlb_crypt::decrypt(char* enciphered, char* output, int length)
{
  ...
  AES aesDecryptor(this->key, this->iv, AES::AES_MODE_128, AES::CIPHER_DECRYPT);
  aesDecryptor.process((uint8_t*)decoded, deciphered, length);
  strcpy(output, (char*)deciphered);
}

/*
String dlb_crypt::encrypt(String text) {
  byte encryptedtext[16];
  byte text_[16];
  text.getBytes(text_, text.length());
  //this->aes.encrypt(encryptedtext,text_);
  String myString = (char*)encryptedtext;
  return myString;
}


String dlb_crypt::decrypt(String text) {
  byte decryptedtext[16];
  byte text_[16];
  text.getBytes(text_, text.length());
  //this->aes.decrypt(decryptedtext,text_);
  String myString = (char*)decryptedtext;
  return myString;
} */